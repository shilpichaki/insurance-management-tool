    ...
    else
    {
        $.ajax({url: "http://localhost:8000/paymentreceive/orderdetails/" + value + "/" + companytype, success: function(result){
            ...
            $.each(result, function(key,value) {
                orders = orders + "<tr>";
                orders = orders +     "<td>";
                orders = orders +         "<input type=\"checkbox\" value=\"\">";
                orders = orders +     "</td>";
                orders = orders +     "<td>" + value.application_no + "</td>";
                orders = orders +     "<td>" + value.applicient_name + "</td>";
                orders = orders +     "<td>" + value.applicient_id + "</td>";
                orders = orders +     "<td>" + value.policy_name + "</td>";
                orders = orders +     "<td>" + value.amount + "</td>"
                orders = orders + "</tr>";
